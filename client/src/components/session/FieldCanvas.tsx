...
export const FieldCanvas: React.FC<FieldCanvasProps> = ({ width: _width, height: _height }) => {
    ...
    // Fit to width (always stable — width comes from CSS layout, not content)
    const widthFitScale = containerWidth > 0 ? containerWidth / totalWidth : 1;

    // On desktop, also constrain by available viewport height to prevent vertical overflow
    // Use window.innerHeight which is stable and doesn't depend on content
    const isDesktop = typeof window !== 'undefined' && window.innerWidth >= 1024;
    const maxStageHeight = isDesktop ? window.innerHeight - 220 : Infinity;
    const heightFitScale = maxStageHeight < Infinity ? maxStageHeight / totalHeight : Infinity;
